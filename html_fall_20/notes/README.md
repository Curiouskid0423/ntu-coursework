...
- In PLA where &nbsp;<img src="https://latex.codecogs.com/gif.latex?\inline&space;sign(w^Tx))" title="sign(w^Tx))" />&nbsp;, if X input is "linearly dependent", Y cannot be shattered. Reasoning [here](https://youtu.be/9kra9i6jS1g?t=4440) with proof by contradiction. 
...
- **Extended Learning**: Intuitively, since both our sample and the population comply to the same joint distribution P(x, y). Still try to derive or check out "*sketch of proof*" of VC bound in a non-deterministic distribution.
